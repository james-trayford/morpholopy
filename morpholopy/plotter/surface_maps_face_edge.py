...
from plotter.helpers import get_stars_surface_density_map, get_gas_surface_density_map, calculate_scaleheight_fit,\
                    get_stars_surface_brightness_map, get_radial_profile, get_Schruba_data, \
                    get_Schruba_upperlimits, sci_notation
...
Sigma_min = 0.
Sigma_max = 99.
H2_over_HI_min = 10.**(-1.9)
H2_over_HI_max = 10.**(+2.4)
...
bingrid = 0.8 * kpc
npix_coarse = int( 2. * r_img_kpc / bingrid )
...
def surface_densities_overview(sim_name, directory, snapshot, catalogue_file, output_path, nhalos, halo_min_stellar_mass, halo_ids_sample):
        ...
        for ihalo, halo_id in enumerate(halo_ids_sample):
                ...
                gs  = gridspec.GridSpec(2, nr_total_plots, wspace = 0.0, hspace = 0.15, height_ratios = [0.05, 1.])

                # General information
                text =  sim_name + ', z = %.1f'%(catalogue.redshift) + '\n\n'
                text += r"${\bf" + "VR\ halo\ id:\ \ \ %3.3i"%(halo_id) + r"}$" + '\n'
                text += r'M$_{\mathrm{200,crit}}$ = ' + sci_notation(catalogue.masses.mass_200crit[halo_id].value) + r' M$_{\odot}$' + '\n'
                text += r'M$_{\mathrm{*,30kpc}}$ = ' + sci_notation(catalogue.masses.mass_star_30kpc[halo_id].value) + r' M$_{\odot}$' + '\n'
                text += r'M$_{\mathrm{gas,30kpc}}$ = ' + sci_notation(catalogue.masses.mass_gas_30kpc[halo_id].value) + r' M$_{\odot}$' + '\n'
                text += r'M$_{\mathrm{HI,30kpc}}$ = ' + sci_notation(catalogue.gas_hydrogen_species_masses.HI_mass_30_kpc[halo_id].value) + r' M$_{\odot}$' + '\n'
                text += r'M$_{\mathrm{H2,30kpc}}$ = ' + sci_notation(catalogue.gas_hydrogen_species_masses.H2_mass_30_kpc[halo_id].value) + r' M$_{\odot}$' + '\n'
                ...
                sSFR = catalogue.apertures.sfr_gas_100_kpc[halo_id].value / 97.78 / catalogue.apertures.mass_star_100_kpc[halo_id].value * 1.e9
                print (halo_id, sSFR)
                if (np.isfinite(sSFR)):
                ...
                mass_map_face, mass_map_edge, visualise_region, x, y, totalmass, H_kpc_gri = get_stars_surface_brightness_map(catalogue, halo_id, snapshot_filename, size, npix, r_img_kpc)
                ...
                circle = plt.Circle((x, y), (0.99 * r_img_kpc.value)/1000., color='black', fill=False, linewidth = 2)
                ax.add_artist(circle)
                ax.plot([x - lbar_kpc/2., x + lbar_kpc/2.], [y + ypos_bar, y + ypos_bar], color = 'white', linewidth = 2, linestyle = 'solid')
                ax.text(x, y + ypos_bar, '%i kpc'%(int(lbar_kpc.value)), color = 'white', \
                ...
                circle = plt.Circle((x, y), (0.99 * r_img_kpc.value)/1000., color='black', fill=False, linewidth = 2)
                ax.add_artist(circle)
                ax.text(0.5, 0.2, r'H$_{r}$ = %.2f kpc'%(H_kpc_gri[1]), ha ='center', va = 'top', color = 'white', transform=ax.transAxes, fontsize = 8)


                # HI face-on
                ax = plt.subplot(gs[nr_total_plots + 3])
                ax.set_title('Gas (HI) - face')
                mass_map_face, mass_map_edge, visualise_region, x, y, totalmass = get_gas_surface_density_map(catalogue, halo_id, 'HI', snapshot_filename, size, npix)
                ...
                ccolor = cmap_loc(0.5)
                im = ax.imshow(mass_map_face_plot, cmap=colormap, extent = visualise_region, vmin = vmin, vmax = vmax)
                circle = plt.Circle((x, y), (0.99 * r_img_kpc.value)/1000., color=ccolor, fill=False, linewidth = 2)
                ax.add_artist(circle)
                ...
                ax.set_yticks([])
                ax.set_axis_off()
                im = ax.imshow(mass_map_edge_plot, cmap=colormap, extent = visualise_region, vmin = vmin, vmax = vmax)
                circle = plt.Circle((x, y), (0.99 * r_img_kpc.value)/1000., color=ccolor, fill=False, linewidth = 2)
                ax.add_artist(circle)
                try:
                        H_kpc = calculate_scaleheight_fit (mass_map_edge.value)
                        ax.text(0.5, 0.2, r'H$_{\mathrm{HI}}$ = %.2f kpc'%(H_kpc), ha ='center', va = 'top', color = 'black', transform=ax.transAxes, fontsize = 8)
                except:
                        H_kpc = -1.
                ...
                mass_map_face, mass_map_edge, visualise_region, x, y, totalmass = get_gas_surface_density_map(catalogue, halo_id, 'H2', snapshot_filename, size, npix)
                mass_map_face.convert_to_units('Msun / pc**2')
                mass_map_edge.convert_to_units('Msun / pc**2')
                ...
                try:
                        H_kpc = calculate_scaleheight_fit (mass_map_edge.value)
                        ax.text(0.5, 0.2, r'H$_{\mathrm{H2}}$ = %.2f kpc'%(H_kpc), ha ='center', va = 'top', color = 'black', transform=ax.transAxes, fontsize = 8)
                except:
                        H_kpc = -1.
                ...
                ax.scatter(S_HI_obs[flag_obs==0] + S_H2_obs[flag_obs==0], S_H2_obs[flag_obs==0] / S_HI_obs[flag_obs==0] , color = 'darkgrey', marker = 'v', facecolor = 'none', label = 'Schruba+2011 (u.limit)')
                S_HI_obs, S_H2_obs, flag_obs = get_Schruba_data()
                ax.scatter(S_HI_obs[flag_obs==2] + S_H2_obs[flag_obs==2], S_H2_obs[flag_obs==2] / S_HI_obs[flag_obs==2] , color = 'lightgrey', marker = '+', label = 'Schruba+2011 (detect.)')

                if totalmass_H2 > 1.e7 * msun:
                        markers        = ['o', 's', 'D']
                        f1 = open(filename_text_f1, 'a')
                        for irad in range(len(radialbinsizes)):
                                radialbin_kpc = radialbinsizes[irad]
                                # datapoint position
                                _, Sigma_HIH2_sub = get_radial_profile(mass_map_face_plot_HI + mass_map_face_plot_H2, radialbin_kpc, pixsize_kpc, r_img_kpc)
                                _, Sigma_HI_sub = get_radial_profile(mass_map_face_plot_HI, radialbin_kpc, pixsize_kpc, r_img_kpc)
                                _, Sigma_H2_sub = get_radial_profile(mass_map_face_plot_H2, radialbin_kpc, pixsize_kpc, r_img_kpc)
                                # datapoint color (metallicity)
                                mass_map_face_plot_H, _, _, _, _, totalmass_H = get_gas_surface_density_map(catalogue, halo_id, 'hydrogen', snapshot_filename, size, npix)
                                mass_map_face_plot_O_diffuse, _, _, _, _, totalmass_O_diffuse = get_gas_surface_density_map(catalogue, halo_id, 'diffuseoxygen', snapshot_filename, size, npix)
                                mass_map_face_plot_star,  _, _, _, _, totalmass_star = get_stars_surface_density_map(catalogue, halo_id, 'stars', snapshot_filename, size, npix)

                                _, Sigma_H_sub = get_radial_profile(mass_map_face_plot_H, radialbin_kpc, pixsize_kpc, r_img_kpc)
                                _, Sigma_O_diffuse_sub = get_radial_profile(mass_map_face_plot_O_diffuse, radialbin_kpc, pixsize_kpc, r_img_kpc)
                                _, Sigma_star = get_radial_profile(mass_map_face_plot_star, radialbin_kpc, pixsize_kpc, r_img_kpc)

                                sc = ax.scatter(Sigma_HIH2_sub, Sigma_H2_sub / Sigma_HI_sub, c = 12. + np.log10 (Sigma_O_diffuse_sub / Sigma_H_sub / 16. ), \
                                           vmin = twelve_plus_logOH_min, vmax = twelve_plus_logOH_max, cmap = cmRdBu, edgecolors = 'black', label = 'Azim.avg. %.2f kpc'%(radialbin_kpc), marker = markers[irad])

                                for i in range (len(Sigma_HIH2_sub)):
                                        if Sigma_H2_sub[i] > 0. and Sigma_HI_sub[i] > 0.:
                                                f1.write(sim_name + '\t' + snapnum + '\t%.2f\t%i\t%.2f\t%.2e\t%.2e\t%.2e\t%.2e\n'%(catalogue.redshift, halo_id, radialbin_kpc, Sigma_HIH2_sub[i], \
                                                                                                                 Sigma_H2_sub[i] / Sigma_HI_sub[i], \
                                                                                                                 12. + np.log10 (Sigma_O_diffuse_sub[i] / Sigma_H_sub[i] / 16. ), Sigma_star[i]))
                        f1.close()
                ...
                if totalmass_H2 > 1.e7 * msun:
                        ...
                        cb.ax.xaxis.set_ticks_position('top')
                        cb.ax.xaxis.set_label_position('top')
                        cb.set_ticks([twelve_plus_logOH_min, twelve_plus_logOH_solar, twelve_plus_logOH_max])
                ...
                mass_map_face_plot_sfr , _, _, _, _, _ = get_gas_surface_density_map(catalogue, halo_id, 'sfr', snapshot_filename, size, npix_coarse)
                mass_map_face_plot_H2  , _, _, _, _, _ = get_gas_surface_density_map(catalogue, halo_id, 'H2', snapshot_filename, size, npix_coarse)
                mass_map_face_plot_HI  , _, _, _, _, _ = get_gas_surface_density_map(catalogue, halo_id, 'HI', snapshot_filename, size, npix_coarse)
                mass_map_face_plot_star, _, _, _, _, _ = get_stars_surface_density_map(catalogue, halo_id, 'Stars', snapshot_filename, size, npix_coarse)
                ...
                for i in range(len(mass_map_face_plot_sfr.ravel())):
                        ...
                        if np.isfinite(logsfr) and np.isfinite(logH2):
                                f2.write(sim_name + '\t' + snapnum + '\t%.2f\t%i\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n'%(catalogue.redshift, halo_id, bingrid.value, logsfr, logH2, logHI, logstar))
